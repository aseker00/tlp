...
public class MaximumLikelihoodEstimationMT extends MaximumLikelihoodEstimation {
	private BlockingQueue<FunctionThread> queue;
	private int threads;

	public MaximumLikelihoodEstimationMT(String name, Tagger tagger, int threads) {
		super(name, tagger);
			...
			while (true) {
				try {
					...
					RelativeFrequencyLogLikelihood step = this.hmm.logLikelihoodStep(sequence,
							MaximumLikelihoodEstimationMT.this);
					this.logLikelihood.additionInc(step);
					MaximumLikelihoodEstimationMT.this.onFunctionThread(this);
